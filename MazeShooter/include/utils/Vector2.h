...
struct Vec2
{
	...
	/*--------------------------------------------*/
	inline friend bool operator<(const Vec2& _main, const Vec2& _other)
	{
		return _main.x < _other.x && _main.y < _other.y;
	}
	...
	inline friend bool operator<=(const Vec2& _main, const Vec2& _other)
	{
		return _main.x <= _other.x && _main.y <= _main.y <= _other.y;
	}
